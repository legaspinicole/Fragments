...
public class EarthToSceneFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_earth_to_game, container, false);
        ...
        tapToContinue.setText("[BREATHE IN]");
    ...
    private void advanceDialogue() {
        if (currentStep == 0) {
            ...
            tapToContinue.setText("[BREATHE OUT]");
            typeText("\"HAS TURNED TO FRAGMENTS.\"");
            currentStep++;
            ...
            typeText("\"BUT THERE'S NOTHING WE CAN'T FIX.\"");
            currentStep++;
        } else {
            playTapSfx();
            fadeToExplosionScene();
        }
    }

    private void fadeToExplosionScene() {
        View rootView = getView();
        if (rootView == null) return;

        // Create fade to black
        View blackFadeView = new View(getContext());
        blackFadeView.setBackgroundColor(0xFF000000);
        blackFadeView.setAlpha(0f);
        ((ViewGroup) rootView).addView(blackFadeView, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));

        ObjectAnimator fadeToBlack = ObjectAnimator.ofFloat(blackFadeView, "alpha", 0f, 1f);
        fadeToBlack.setDuration(500);
        fadeToBlack.addListener(new android.animation.AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(android.animation.Animator animation) {
                // Navigate to explosion scene
                if (getActivity() != null) {
                    ExplosionChoicesFragment explosionFragment = new ExplosionChoicesFragment();
                    FragmentTransaction transaction = getActivity().getSupportFragmentManager().beginTransaction();
                    transaction.setCustomAnimations(android.R.anim.fade_in, android.R.anim.fade_out);
                    transaction.replace(R.id.fragment_container, explosionFragment);
